        ...
        <div className="table-wrap"><table data-stock-table>
          ...
          <tbody>{rows.map(r => (
            <tr key={r.variant_id} data-stock-row={r.variant_sku} data-level={r.is_active ? r.stock_status : 'off'}>
              ...
              <td className="nowrap note">{formatDateTime(r.last_movement_at)}</td>
              <td className="num"><Link className="btn ghost sm" href={`/products/${r.product_id}#stock-h`} aria-label={`Adjust ${r.variant_sku}`}>Adjust</Link></td>
            </tr>))}
          </tbody>
        </table></div>
      )}
